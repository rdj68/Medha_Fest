document.addEventListener("DOMContentLoaded", function () {
  ...
  centralMembers.forEach((member) => {
    ...
    mainDiv.className =
      "flex cursor-pointer hover:scale-105 transition-all duration-300 ease-out justify-center rounded-lg items-center p-2 gap-4 border-[1px] backdrop-blur-sm bg-white/30 border-gray-500";
    let image = document.createElement("img");
    image.className = "h-[130px] object-contain w-[130px] rounded-full";
    image.alt = member.name;
    image.src = `../members/developers/${member.name}.png`;
